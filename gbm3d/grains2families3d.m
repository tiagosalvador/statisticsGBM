function [families,famgrains] = grains2families3d(grains,R,dims)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [families,famgrains] = grains2families3d(grains,R,dims)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
collect_cval1 = zeros(n1*n2*n3,1);
collect_cval2 = zeros(n1*n2*n3,1);
label = 0;
while N>sum(remaininggrains>0) % Loop over families.
    ...
    for k=1:N % Loop over the grains.
        ...
        if (intersects(k)==0 && remaininggrains(k)==0) % If grain k is unallocated
            ...
            [x2,y2,z2] = dilation(int32(x),int32(y),int32(z),R,WORKSPACE,LONGARRAY); % Dilation.
