function [areas_grains_slices, areas_convhull_slices, nNeighbors_slices,...
    mdf_slices, isopratio_slices] = ...
    compute_statistics_slices3D(grains,dims,ori,Rfamilies,dt,spacing)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mdf_conv_slices = cell(length(dims),length(slices));
mdf_pixels_slices = cell(length(dims),length(slices));
...
for i = 1:length(dims)
    ...
    for s = 1:length(slices)
        ...
        [areas_grains_slices{i,s},areas_convhull_slices{i,s},...
            nNeighbors_slices{i,s},mdf_slices{i,s},...
            isopratio_slices{i,s}] = compute_statistics_slices3D_aux(grains_slices,ID_slices,dims_slices,minang,dt);
